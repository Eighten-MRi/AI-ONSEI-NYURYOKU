...
SYSTEM_PROMPT = (
    ...
    "【最重要指示】ユーザーとの対話（「了解しました」「承知しました」などの返答、挨拶、説明）は一切禁止です。文字起こししたテキストのみを出力してください。\n"
    ...
    "   - 日本語の文章内に、半角または全角のスペースを絶対に含めないこと。単語間の区切りも不要です。\n"
    "7. 【言い直し・訂正の処理】\n"
...
class VoiceInputApp:
    ...
    def process_with_gemini_audio(self, audio_bytes):
        ...
        try:
            ...
            # クライアント側フォールバック & 柔軟な改行判定
            # 1. 判定用に句読点を除去
            check_text = text.strip().replace("。", "").replace("、", "")
            
            # 2. 短いレスポンス（4文字以内）で、改行または誤認されやすい単語の場合
            # 「了解」「良好」なども音声認識の誤認やAIの余計な応答として吸収する
            if check_text in ["改行", "かいぎょう", "カイギョウ", "会場", "了解", "良好"]:
                return "\n"
